...
- check timestamps;
- parser version;
- the `comparisonBaseline` fingerprint and the last observed normalized fingerprint, kept distinct and compared only through their `comparisonDigest`;
...
`manifest.json` is the initialization registry (per-source `lifecycle` and `everInitialized`) that separates a never-initialized monitor from an initialized source whose state was lost; it and the affected per-source files update atomically in one state commit. That branch is the authoritative durable store — GitHub Actions artifacts/caches are a transient optimization only. It is never merged into `main`, never opened as a review Pull Request, and never triggers a deployment; it carries compact metadata and hashes only, never fetched source bodies, secrets, AI prompts or long excerpts. On `main`, `evidence/state/` stays the empty placeholder directory that owns the path convention (§2), and Phase 9 adds a repository-health/baseline check that keeps populated `manifest.json`/`sources/**` files off `main` (`REPOSITORY_HEALTH.md` §10). Evidence review branches are a separate namespace, `evidence-watch/review/<sourceId>`, and are never used as the state store. The full state schema and its transitions are `EVIDENCE_UPDATE_ENGINE.md` §21.

Because it is authoritative, the branch has its own write and protection rules: every write is serialized behind a single state-writer concurrency group and uses fast-forward compare-and-swap semantics (workflow concurrency is scheduling assistance, not the transactional mechanism), a per-source state file commits atomically with any related `manifest.json` change, a stale write reloads the latest head and reapplies rather than overwriting, and the branch is never force-pushed. Its history is append-only recovery history — no squash, reset or rewrite — and a restore is itself a new commit. Phase 9 configures a ruleset on `evidence-watch/state` blocking force-push and deletion and restricting writes to the Evidence Watch identity and an explicit maintainer recovery path; that protection is separate from the `main` ruleset.
...
- a required deterministic source freshness check blocks merging an Evidence Watch review Pull Request whose source moved past the reviewed fingerprint, and an idempotent post-merge reconciliation advances the watcher baseline only against that verified Pull Request head;
