...
- durable watcher operational state on the `evidence-watch/state` branch (§9), with explicit manual `bootstrap`, `rebaseline` and `reconcile` dispatch modes — a scheduled run never establishes or replaces a baseline by itself, and reconciles outstanding merged reviews before classifying anything;
