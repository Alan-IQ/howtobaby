...
Watcher state — fingerprints, check metadata, cache — is its own operational plane. Canonical `SourceRecord`/claim/provenance state changes only through the reviewed merge path, and an unresolved detected change lives in the Draft Pull Request rather than in production (`EVIDENCE_UPDATE_ENGINE.md` §11, §13).
