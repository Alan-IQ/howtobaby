...
- a compact deterministic `SourceObservation` (availability, normalized effective URL, optional fingerprint, locator states, bounded classification signals) as the identity of the **complete observed source condition**, frozen at `sourceObservationDigestVersion = "sha256-v1"` and domain-separated by `digestType: "source-observation-v1"`, with every source-side fact a classification uses represented in it;
