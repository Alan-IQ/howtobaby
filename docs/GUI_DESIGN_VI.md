...
2. **EvidenceDrawer** khi click: exact source title, relationship badge, rồi metadata theo đúng thứ tự cố định: (1) `Phần liên quan` — section/page locator; (2) `Áp dụng cho` / `Phạm vi`; (3) metadata phát hành/phiên bản nguồn theo ma trận điều kiện (EVIDENCE_PROVENANCE.md §14): chỉ `publishedAt` → `Phát hành: …`; chỉ `updatedAt` → `Phiên bản nguồn hiện tại: 14/04/2026`; có cả hai và bằng nhau → chỉ `Phát hành: …` (không lặp cùng ngày thành `Cập nhật`); có cả hai và `updatedAt` muộn hơn → `Phát hành: 10/01/2025` + `Cập nhật: 14/04/2026`; không có cả hai → bỏ hẳn, không suy đoán ngày; (4) `HowToBaby kiểm chứng lần cuối: 31/08/2026` — ngày review workflow của HowToBaby thực sự xác nhận nguồn, không phải crawl hay deploy time; (5) `Vì sao dùng nguồn này`; (6) `Xem nguồn gốc`. Badge trạng thái CHỈ render khi cần chú ý (`changed-review-required`, `superseded`, `retired`, `temporarily-unreachable`); source `current` lành mạnh không có status UI. Interpretation/conflict note đi sau khi cần. References (`Nguồn dùng trên trang này`) nối cùng các hàng đó trên một dòng (`Phát hành: … · Cập nhật: … · HowToBaby kiểm chứng lần cuối: …`), không thêm hàng để list không nặng.
