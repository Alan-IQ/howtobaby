...
- mỗi source chỉ có một review chưa resolve: PR lịch sử đã merged/closed không bao giờ được adopt làm review hiện tại, branch của review đã resolve được dọn dẹp idempotent sau khi reconcile state thành công, và evidence event kế tiếp của source đó bắt đầu bằng branch mới dựng từ `main` hiện tại;
