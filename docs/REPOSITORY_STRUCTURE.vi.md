...
- watcher operational state bền trên branch `evidence-watch/state`, có manual dispatch mode `bootstrap`, `rebaseline` và `reconcile` tường minh — scheduled run không bao giờ tự lập hay thay baseline, và reconcile các merged review tồn đọng trước khi classify;
...
- một required deterministic source freshness check chặn merge khi source đã đi quá fingerprint đã review, và một post-merge reconciliation idempotent chỉ advance baseline theo đúng PR head đã verify;
