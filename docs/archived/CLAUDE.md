...
- Use English for code comments, public APIs, identifiers, filenames, commit messages, canonical technical documentation, and canonical product content unless the project contract says otherwise.
- Be direct, practical, architecture-aware, evidence-aware, and privacy-aware. Do not over-explain obvious details.
- State a concise plan before large, destructive, security-sensitive, medically sensitive, or non-trivial changes.
- Ask only when a missing decision genuinely blocks safe/correct progress; otherwise make the smallest conservative assumption and proceed.
- After editing, summarize changed files concisely.
- NEVER claim a command, build, test, deployment, content validation, source verification, or evidence check succeeded unless it was actually executed successfully.

## 2. Prime directive

> **Do not publish, recommend, or implement a conclusion until it is the best available conclusion under the known evidence, product contracts, and safety constraints.**

Before non-trivial work:

1. Understand the requested outcome and relevant scope/phase.
2. Inspect the nearest implementation/tests/configuration plus only the canonical docs needed.
3. Verify material assumptions, especially health/safety/source/age claims.
4. Consider a meaningful simpler or safer alternative when one exists.
5. Challenge the current answer for invented precision, hidden medical inference, duplicated authority, unnecessary backend/abstraction, privacy leakage, and architecture drift.
6. Make the smallest complete change that preserves product integrity.

Never optimize for agreement over correctness.
...
- `docs/PROJECT_PROFILE_v0.7.0.md` — mission, scope, product principles, non-negotiable decisions.
