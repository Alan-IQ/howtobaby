...
- Evidence Watch operational state is persisted on the dedicated non-canonical `evidence-watch/state` branch (`evidence/state/**`), never merged into `main` and never canonical. Its `comparisonBaseline` is distinct from the last observed fingerprint, both compared only through the deterministic `comparisonDigest` (never `checkedAt` or other observation metadata), and it never advances without a valid resolution verified against the merged Pull Request head. Bootstrap, rebaseline and reconcile are explicit manual operations, never something a scheduled run does silently; an initialized source is never re-bootstrapped to recover lost state; state-sync failure fails closed; and writes to that branch are serialized and never force-pushed.
