...
**v0.8.0 — Phase 0 (repository baseline), Phase 1 (app shell + theme engine), Phase 2 (content/source schema platform), Phase 3 (age/context + browse/public routing) and Phase 4 (Play & Development domain) implemented; Phase 5+ (Sleep, Feeding domains, the personalized Now composer, tools, Evidence Watch) not yet started.**
...
Phase 1 adds the Next.js + TypeScript application in `apps/web` (static-first, server-capable; `DEPLOY_TARGET=static` is an optional export profile), the vendor-neutral Theme Contract/Registry and css-vars adapter in `packages/themes` with the first-party **Baby Modern Glass** Light/Dark packs and a third-party adapter fixture, the semantic-token UI primitives in `packages/ui`, the AppShell/Header/primary navigation/page shell with responsive, reduced-motion/transparency and print scaffolding, a locally stored theme preference, the theme-boundary CI gate, and the production deployment pipeline to `https://howtobaby.com` (the `deploy-production` job of `.github/workflows/pipeline.yml`, runbook in [`DEPLOYMENT_HAWKHOST.md`](DEPLOYMENT_HAWKHOST.md)).

Phase 2 adds the canonical knowledge graph in `packages/knowledge` — Git-tracked YAML for sources, claims, guidance blocks and EN/VI translations, with schema/source-approval/provenance/translation-parity/coverage validators and a release gate — compiled into derived, rebuildable read models (`knowledge.sqlite`, JSON manifests and the claim/source/route/tool evidence indexes, all gitignored) behind the `KnowledgeRepository` interface. The app renders its first canonical sample guidance (starting solids, on `/feeding`) in English and Vietnamese from that graph, with SourceChip, Evidence Drawer, page References, the `/sources` registry and `/evidence/[slug]` detail pages all fed from one provenance graph; a global EN/VI language preference with a per-card canonical-language toggle; and a deterministic delete-and-rebuild CI gate for every derived artifact. Only that sample claim is published — full domain content (Feeding, Play & Development, Sleep, Safety), the personalized Now view, Tools and Evidence Watch belong to later phases. Pre-v1, every page ships `noindex` (indexing is deliberately opened in a later phase), and production UI exposes only Baby Modern Glass — the vendor fixture theme exists solely for theme-independence testing via the development-only Theme Lab.

Phase 3 adds the deterministic age/context layer in `packages/core` — calendar-date utilities on timezone-independent day serials, chronological age, the corrected-development proxy exactly as the content contract defines it, the Development/Feeding/Sleep stage bins, and the actual/browsed/preview `GuidanceContext` model in which safety-sensitive logic can read only the actual child — plus, in `apps/web`, statically generated public age routes (`/play/<stage>`, `/feeding/<stage>`, `/sleep/<stage>`, one page per stage bin, no request-time slugs), a StageNavigator and "Why this stage" card on every stage page, "Browse by age" on the domain pages, and an optional local child profile (date of birth required; due date and name optional; localStorage only) that powers the child summary and a session-only plan-date preview on Now. Browsing needs no profile, and a child's exact data never enters a URL, document metadata or prerendered HTML.

Phase 4 migrates Play & Development into the canonical content graph: 24 audited sources (CDC's twelve "Learn the Signs. Act Early." milestone checklists and overview, CDC positive-parenting/safety pages per age band, AAP corrected-age, tummy-time and baby-walker guidance, the AAP "Power of Play" report and the WHO under-5 activity guideline), 173 source-verified claims covering every one of the 12 Development stages × the 14 contract sections (at a glance, milestone context, development focus, gross motor, fine motor/hand–eye, communication, cognitive, social-emotional, activities, easier/harder variations, safety/environment, what not to force, what to observe, clinician cues) plus a cross-stage "how to read milestones" block, all in English and Vietnamese with a coverage cell per stage. `@howtobaby/core` adds CDC checklist resolution (an age between two checklist ages uses the younger checklist; none under 2 months; a corrected age before the due date resolves to no stage), `/play/<stage>` renders the stage graph in contract order with the reference checklist in Why-this-stage, `/play/all-stages` is the printable all-stage reference, and milestones render as reference lists — never checkboxes, scores or deadlines (gated in both locales).
...
Requirements: Node.js `>= 24` (`.nvmrc`) and pnpm 11 — run `corepack enable` once and pnpm resolves to the version pinned in `package.json` → `packageManager`.
